...
namespace Ktbartholomew\SamlPlugin\Admin;

use \stdClass;
use Ktbartholomew\SamlPlugin\Config\Wordpress as WordpressConfig;
...
Class Dashboard {
  ...
  {
    $this->settings = WordpressConfig::getConfig();
  ...
  {
      $idp_details = null;
      $idp_settings = $this->settings['idp_details'];
  ...
  */
  public function get_saml_status()
  {
    ...
    $status = array(
      'idp_entityid' => array(
          'error_default' => 'You have not changed your IdP’s Entity ID from the default value. You should update it to a real value.',
          'error_blank'   => 'You have not provided an Entity ID for your IdP.',
          'warning'       => 'The Entity ID you provided may not be accessible (perhaps a bad URL). You should check that it is correct.',
     ...
     */
    $hasKeysInDb = ($this->settings['certificate']['public_key'] && $this->settings['certificate']['private_key']);
    ...
    if( trim($attributes['username']) == '' || trim($attributes['groups']) == '' )
    {
      $return->html .= $status_html['error'][0] . $status['sp_attributes']['error'] . $status_html['error'][1];
      $return->num_errors++;
    }
    elseif(trim ($attributes['firstname']) == '' || trim ($attributes['lastname']) == '' || trim ($attributes['email']) == '')
    {
    ...
    if( trim($groups['administrator']) != '' )
    {
      $return->html .= $status_html['ok'][0] . $status['sp_permissions']['ok'] . $status_html['ok'][1];
    }
    elseif(trim($groups['administrator']) == '' && (trim($groups['editor']) != '' || trim($groups['author']) != '' || trim($groups['contributor']) != '' || trim($groups['subscriber']) != '') )
    {
  ...
  public function sso_general(){
    include(__DIR__ . '/controllers/' . __FUNCTION__ . '.php');
  }
