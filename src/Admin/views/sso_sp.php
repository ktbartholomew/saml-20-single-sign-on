<div class="wrap">
<?php
  if($this->settings['idp_details'] === FALSE) {
...
<table class="form-table">
  <input type="hidden" name="idp" id="idp" value="<?php echo $this->settings['idp']; ?>" />
  <tr valign="top">
    ...
    <td>
      ...
      <?php
          ...
          {
            $selected = ( $this->settings['nameidpolicy'] == $policy ) ? ' selected="selected"' : '';
  ...
  <tr valign="top" class="manual_cert">
    ...
    <?php
             ...
             */
            if($this->settings['certificate']['public_key'] && $this->settings['certificate']['private_key']) {
              $certificate = $this->settings['certificate']['public_key'];
	            $certificate_cn = openssl_x509_parse($certificate);
	            $certificate_cn = $certificate_cn['subject']['CN'];
	            $privatekey = $this->settings['certificate']['private_key'];
        ...
        ?>
    <td><input type="file" name="certificate" id="certificate" /><?php if($certificate !== false ) {echo '&nbsp;<span class="green">Using certificate: <strong>' . $certificate_cn . '</strong>.</span>';}?>
    <br/>
...
<table class="form-table">
  ...
  <tr valign="top">
    ...
    <td><input type="text" name="username_attribute" id="username_attribute_inp" value="<?php echo $this->settings['attributes']['username']; ?>" size="40" data-if-empty="error" />
    </td>
  </tr>

    <tr valign="top">
    <th scope="row"><label for="firstname_attribute">Attribute to be used as First Name</label></th>
    <td><input type="text" name="firstname_attribute" id="firstname_attribute_inp" value="<?php echo $this->settings['attributes']['firstname']; ?>" size="40" data-if-empty="warning" />
    </td>
  </tr>

    <tr valign="top">
    <th scope="row"><label for="lastname_attribute">Attribute to be used as Last Name</label></th>
    <td><input type="text" name="lastname_attribute" id="lastname_attribute_inp" value="<?php echo $this->settings['attributes']['lastname']; ?>" size="40" data-if-empty="warning" />
    </td>
  </tr>

    <tr valign="top">
    <th scope="row"><label for="email_attribute">Attribute to be used as E-mail</label></th>
    <td><input type="text" name="email_attribute" id="email_attribute_inp" value="<?php echo $this->settings['attributes']['email']; ?>" size="40" data-if-empty="warning" />
    </td>
  </tr>
  <tr valign="top">
    <th scope="row"><label for="groups_attribute">Attribute to be used as Groups</label></th>
    <td><input type="text" name="groups_attribute" id="groups_attribute_inp" value="<?php echo $this->settings['attributes']['groups']; ?>" size="40" data-if-empty="error" />
    </td>
  </tr>
  </table>
  <h3>Groups</h3>
  ...
  <th><label for="<?php echo $role_name ?>_entitlement"><?php echo $role_meta['name'] ?>s Group Name</label></th>
    <td><input type="text" name="<?php echo $role_name ?>_group" id="<?php echo $role_name ?>_group" value="<?php echo $this->settings['groups'][$role_name]; ?>" size="40" data-if-empty="warning" /><br/>
  ...
  <tr>
    ...
    <td><input type="checkbox" name="allow_unlisted_users" id="allow_unlisted_users" value="allow" <?php echo ($this->settings['allow_unlisted_users']) ? 'checked="checked"' : ''; ?> /><br/>
